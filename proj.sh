#! /bin/bash
printf "napisz 1 jesli chcesz wykonac konwersje tylko  na jeden system, w przypadku konwersji na wiele systemow napisz 2\n"
read -r wybor
if [ $wybor == 1 ]
then
...
then
	printf "w odstepach podaj na jakie systemy, ktore beda uzyte do wyliczen (system obliczy wartosci w kolejnosci od pierwszej do ostatniej) \n"
        read -r c
        lista=($c)
        dlugosc=${#lista[@]}
        for (( z=0; z<$dlugosc; z++))
